...
testImages = 1
black_area_width   = int((imageSize[0]-720/(1280/imageSize[0]))/2)
black_area  = int(int(imageSize[0]-720/(1280/imageSize[0]))*imageSize[0]/2)
limit = imageSize[0]*imageSize[1]
point = np.genfromtxt('testPoints_box.txt')

for i in range(testImages) :
	print(i)
	index = imageSize[0]*black_area_width
	cnt = imageSize[0]*black_area_width
	outputImage = cv2.imread('testImages_box/image_'+str(i)+'_'+str(index - imageSize[0]*black_area_width)+'.png')
	while not index == limit-1 - black_area :
		print('index is %d'%(index-imageSize[0]*black_area_width))
		inputImages = cv2.imread('testImages_box/image_'+str(i)+'_'+str(index- imageSize[0]*black_area_width)+'.png')
		...
		if output[0][0] > output[0][1] :
			if (index+1)%imageSize[0] == 0 :
				print('over side: wrong grid entered')
				break
		...
		if (index)>=(limit - black_area) or point[i*limit+index] == -100 :
			print('wrong grid entered', (index- imageSize[0]*black_area_width))
			break
		else :
