...
ene_pos = end_pos_x*height+width-1
...
if __name__ == '__main__':
    ...
    for t in range(totalGames) :
        ...
        # print(end_pos_x)
        # breakpoint()
        for i in range(end_pos_x-start_pos_x+1) :
            for j in range(x):
                a[i+start_pos_x][j][0] = 0
                a[i+start_pos_x][j][1] = 255
                a[i+start_pos_x][j][2] = 0
                cv2.imwrite(os.path.join(folderName,"image_"+str(t)+"_"+str(gridSize[0]*i+j)+".png"),a)
                if flag[(i+start_pos_x)*x+j] == 0 : 
                    a[i+start_pos_x][j][0] = 255
                    a[i+start_pos_x][j][1] = 255
                    a[i+start_pos_x][j][2] = 255
                else :
                    a[i+start_pos_x][j][0] = 0
                    a[i+start_pos_x][j][1] = 0
                    a[i+start_pos_x][j][2] = 0
