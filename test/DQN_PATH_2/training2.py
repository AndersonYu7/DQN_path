...
black_area   = int(imageSize[0]-720/(1280/imageSize[0]))*imageSize[0]
black_area_width   = int((imageSize[0]-720/(1280/imageSize[0]))/2)
totalImages = games * (imageSize[0] * imageSize[1] - black_area)

#gamesArr = np.arange(totalImages)

start = 0
# start = black_area_width*imageSize[0]
def randomiseGames() :
	global gamesArr
...
def getInput(index) :
	...
	temp = imageSize[0]*imageSize[1] - black_area
...
def getBottomOutput(index,points,target) :
	...
	if (pos+imageSize[0]>=(limit - int(black_area/2))) or (points[index+imageSize[0]] == -100) :
		#print 'hi'
		output = -100
...
def getBatchInputOutput(batchSize,points,target) :	
	...
	if start > len(gamesArr) :
		...
		randomiseGames()
		# first = imageSize[0]*black_area_width
		first = 0
		start = batchSize
	end = start
